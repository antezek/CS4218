...
public class UNIQToolTest {
	...
	/**
	 * Additional test case to test readFromFile method when reading from valid file
	 */
	@Test
	public void readFromValidFileTest() {
		actualOutput = readFromFile(inputFile1);
		expectedOutput = "hi\nhi\nhello\nhello\nhi\nabc";
		assertEquals(expectedOutput, actualOutput.trim());
		
		actualOutput = readFromFile(inputFile3);
		expectedOutput = "* abc hi\n(( hi hi\n[] l hi";
		assertEquals(expectedOutput, actualOutput.trim());
	}
	
	/**
	 * Additional test case to test readFromFile method when reading from invalid file
	 */
	@Test
	public void readFromInvalidFileTest() {
		File invalidFile = new File("Invalid.txt");
		actualOutput = readFromFile(invalidFile);
